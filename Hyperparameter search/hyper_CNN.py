...
# #### This is the hyperparameter search file for the CNN.
...
space = {
    ...
    'kernel_x': hp.choice('kernel_x', np.arange(1, 9, dtype=int)),
    'kernel_y': hp.choice('kernel_y', np.arange(1, 9, dtype=int)),
    'strides_x': hp.choice('strides_x', np.arange(1, 4, dtype=int)),
...
def CNN_model(param):
    ...
    CNN.add(layers.Conv2D(param['filters'], strides=(param['strides_x'],param['strides_y']), kernel_size=(param['kernel_x'],param['kernel_y']), activation='relu', kernel_initializer= initializers.he_normal(), input_shape=(n_timesteps,n_features,1)))#height, width, channels
    CNN.add(layers.BatchNormalization())
    CNN.add(layers.Flatten())
    CNN.add(layers.Dense(param['unitsDense'], activation='relu', kernel_initializer= initializers.he_normal()))
    ...
    mc = ModelCheckpoint('saved_models/CNN_best.h5', monitor='val_acc', mode='max', verbose=1, save_best_only=True)
    #train CNN2D
    history = CNN.fit(data_train, labels_train, validation_split=0.11, batch_size=param['batch'], epochs=100, verbose=0, callbacks=[es, mc])
    #load best model
    CNN_best = load_model('saved_models/CNN_best.h5')
    #predict outcome for confusion matrix
    predict_classes = CNN_best.predict_classes(data_test, verbose=0)
    # accuracy: (tp + tn) / (p + n)
    scores = (accuracy_score(labels_test.argmax(axis=1), predict_classes))*100
    # clear tf backend
    if K.backend() == 'tensorflow':
        K.clear_session()
    del n_features, n_outputs, CNN, es, mc, history, CNN_best, predict_classes
    global counter
    counter += 1
...
if __name__ == '__main__':
    ...
    trials = Trials()
    best = fmin(CNN_model, space, algo=tpe.suggest, trials=trials, max_evals=500, verbose=0)
    ...
    print('Working directory: ' + os.getcwd())
    filename =  'hyper_CNN.csv'
    with open(filename, 'w') as f:
        thewriter = csv.writer(f)
        thewriter.writerow(['Hyperparameter search','CNN','duration',duration])
        for i in range(0,best_array.shape[0]):
            thewriter.writerow(best_array[i,:])
        thewriter.writerow(['Best trial',trials.best_trial])
        thewriter.writerow(['All trials',trials.trials])
        f.close()
