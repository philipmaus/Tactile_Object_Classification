...
# #### This is the hyperparameter search file for the CNNLSTM.
...
space = {
    ...
    'kernel': hp.choice('kernel', np.arange(1, 9, dtype=int)),
    'strides': hp.choice('strides', np.arange(1, 4, dtype=int)),
...
def CNNLSTM_model(param):
    n_timesteps, n_features, n_outputs = data_train.shape[1], data_train.shape[2], labels_train.shape[1]
    #NN layers
    CNNLSTM = keras.Sequential()
    CNNLSTM.add(layers.TimeDistributed(layers.Conv1D(filters=param['filters'],kernel_size=(param['kernel'],),strides=(param['strides'],),activation='relu', kernel_initializer= initializers.he_normal(),input_shape=(n_timesteps,n_features,1))))
    CNNLSTM.add(layers.TimeDistributed(layers.Dropout(param['dropout1'])))
    CNNLSTM.add(layers.TimeDistributed(layers.Flatten()))
    CNNLSTM.add(layers.LSTM(param['unitsLSTM']))
    CNNLSTM.add(layers.Dropout(param['dropout2']))
    ...
    mc = ModelCheckpoint('saved_models/CNNLSTM_best.h5', monitor='val_acc', mode='max', verbose=1, save_best_only=True)
    #train CNNLSTM
    history = CNNLSTM.fit(data_train, labels_train, validation_split=0.11, batch_size=param['batch'], epochs=100, verbose=0, callbacks=[es, mc])
    #load best model
    CNNLSTM_best = load_model('saved_models/CNNLSTM_best.h5')
    #predict outcome for confusion matrix
    predict_classes = CNNLSTM_best.predict_classes(data_test, verbose=0)
    # accuracy: (tp + tn) / (p + n)
    scores = (accuracy_score(labels_test.argmax(axis=1), predict_classes))*100
    # clear tf backend
    if K.backend() == 'tensorflow':
        K.clear_session()
    del n_features, n_outputs, CNNLSTM, es, mc, history, CNNLSTM_best, predict_classes
    global counter
    counter += 1
...
if __name__ == '__main__':
    ...
    trials = Trials()
    best = fmin(CNNLSTM_model, space, algo=tpe.suggest, trials=trials, max_evals=500, verbose=0)
    ...
    print('Working directory: ' + os.getcwd())
    filename =  'hyper_CNNLSTM.csv'
    with open(filename, 'w') as f:
        thewriter = csv.writer(f)
        thewriter.writerow(['Hyperparameter search','CNNLSTM','duration',duration])
