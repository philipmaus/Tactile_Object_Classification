...
# #### This is the hyperparameter search file for the MLP.
...
space = {
    'units1': hp.choice('units1', np.arange(8, 1024, dtype=int)),
    'units2': hp.choice('units2', np.arange(8, 1024, dtype=int)),
    'dropout1': hp.uniform('dropout1', 0.001, 1),
    'dropout2': hp.uniform('dropout2', 0.001, 1),
...
def MLP_model(param):
    n_features, n_outputs = data_train.shape[1], labels_train.shape[1]
    #NN layers
    MLP = keras.Sequential()
    MLP.add(layers.Dense(param['units1'], input_dim=n_features, activation='relu', kernel_initializer= initializers.he_normal()))  # Adds a densely-connected layer with 644 units to the model:
    MLP.add(layers.Dropout(param['dropout1']))                   # dropout layer with rate 0.62
    MLP.add(layers.Dense(param['units2'], activation='relu', kernel_initializer= initializers.he_normal()))
    MLP.add(layers.Dropout(param['dropout1']))
    ...
    #optimizer: standard: adam(0.01), Kerzel: adadelta(1), batch size 76
    # simple early stopping
    es = EarlyStopping(monitor='val_loss', mode='min', verbose=0, patience=50)
    mc = ModelCheckpoint('saved_models/MLP_best.h5', monitor='val_acc', mode='max', verbose=1, save_best_only=True)
    #train MLP
    history = MLP.fit(data_train, labels_train, validation_split=0.11, batch_size=param['batch'], epochs=100, verbose=0, callbacks=[es, mc])
    #load best MLP model
    MLP_best = load_model('saved_models/MLP_best.h5')
    #predict outcome for confusion matrix
    predict_classes = MLP_best.predict_classes(data_test, verbose=0)
    # accuracy: (tp + tn) / (p + n)
    scores = (accuracy_score(labels_test.argmax(axis=1), predict_classes))*100
    # clear tf backend
    if K.backend() == 'tensorflow':
        K.clear_session()
    del n_features, n_outputs, MLP, es, mc, history, MLP_best, predict_classes
    global counter
    counter += 1
...
if __name__ == '__main__':
    ...
    trials = Trials()
    best = fmin(MLP_model, space, algo=tpe.suggest, trials=trials, max_evals=1000, verbose=0)
    ...
    print('Working directory: ' + os.getcwd())
    filename =  'hyper_MLP.csv'
    with open(filename, 'w') as f:
        thewriter = csv.writer(f)
        thewriter.writerow(['Hyperparameter search','MLP','duration',duration])
